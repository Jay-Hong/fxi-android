 ...
 * There is deliberately no RECOVERY_INTENT overload in 5d.
 */
internal fun planRecoveryRetirement(
    source: HoldRecoverySource,
    before: FenceV1,
    ...
    val axes = source.axes.sortedBy { it.ordinal }.map { axis ->
        val epoch = source.subject.epoch(axis)
        ...
        RecoveryAxisRetirement(JournalTargetV1(source.subject.ownerUid, axis, epoch), action)
    }
    ...
    val plan = RecoveryRetirementPlan(before, after, axes, journal,
        if (source.subject.ownerUid == before.ownerUid) RecoveryRequestRequirement.RequiredCurrentOwner
...
internal object RecoveryRetirementBoundary {
    ...
    fun axisProblem(source: HoldRecoverySource, axis: PurgeScope, before: FenceV1): HoldRecoveryProblem? {
        if (axis !in source.axes) return invalid("SourceAxisMismatch") // HR.axisMember
        val target = source.subject.epoch(axis)
        ...
        if (source.subject.ownerUid != before.ownerUid && target != null && target == current)
            return HoldRecoveryProblem.Conflict(ConflictReason.TargetChanged) // HR.departedEqual
        return null
    }

    internal fun rotationRequired(source: HoldRecoverySource, axis: PurgeScope, before: FenceV1): Boolean =
        source.subject.ownerUid == before.ownerUid &&
            (source.subject.epoch(axis) == null || source.subject.epoch(axis) == before.epoch(axis))

    internal fun inputProblem(source: HoldRecoverySource, before: FenceV1, fresh: RecoveryFreshEpochs): HoldRecoveryProblem? {
        ...
        if (!journalFieldRepresentable(source.subject.ownerUid)) return invalid("UnrepresentableJournalField") // HR.journalOwnerCall
        val issued = mutableListOf<String>()
        for (axis in source.axes) {
            axisProblem(source, axis, before)?.let { return it } // HR.axisCall
            if (!journalFieldRepresentable(source.subject.epoch(axis))) return invalid("UnrepresentableJournalField") // HR.journalEpochCall
    ...
    fun validPlan(
        ...
        val axes = source.hold.axes.sortedBy { it.ordinal }
        ...
        for (axis in PurgeScope.entries) {
            val included = axis in source.hold.axes
            ...
            if (included) {
                ...
                if (actual.target.ownerUid != source.subject.ownerUid) return false // HR.planTargetOwner
                if (actual.target.epoch != source.subject.epoch(axis)) return false // HR.planTargetEpoch
                ...
                expectedJournal += PendingPurge(source.subject.ownerUid,
        ...
        val requirement = if (source.subject.ownerUid == before.ownerUid) RecoveryRequestRequirement.RequiredCurrentOwner
            else RecoveryRequestRequirement.NotRequiredDepartedOwner

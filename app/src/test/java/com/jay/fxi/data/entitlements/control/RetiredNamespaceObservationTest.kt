...
class RetiredNamespaceObservationTest : RetiredNamespaceOwnerBase() {
    ...
    @Test fun A06_firstObservationWithOpaqueSibling_departedWitnessRetained() = runReleaseTest {
        firstObservationThenLoss(departed(), restorePreimage = false)
    }

    @Test fun A06_firstObservationWithOpaqueSibling_departedPreimageRestored() = runReleaseTest {
        firstObservationThenLoss(departed(), restorePreimage = true)
    }

    private suspend fun firstObservationThenLoss(s: RetiredNamespaceSettlement, restorePreimage: Boolean) {
        val initial = raw(s)
        seedR(s, initial)
        val c = registerR(s)
        ...
        controlTestTimeout("R opaque sibling after landing") { o.data.updateData { opaque } }
        val recoveryWrites = o.storage.writes
        val recovery = executeR(c, ctx)
        ...
        controlTestTimeout("R repair sibling and lose Applied") { o.data.updateData { restored } }
        val lostWrites = o.storage.writes
        val lost = executeR(c, ctx)

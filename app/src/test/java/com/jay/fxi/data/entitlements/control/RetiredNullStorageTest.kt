...
class RetiredNullStorageTest : RetiredNullOwnerBase() {
    private suspend fun failure(s: RetiredNullSettlement, boundary: String, restart: Boolean = false) {
        val source = raw(s); seedL(s, source); val c = registerL(s)
        when (boundary) {
        ...
        if (restart) {
            ...
            val previous = controlTestTimeout("previous L witness") { next.control.confirmPrevious(c) }
            assertNull(nt.findPrepared(c)); assertFalse(c in nt.executing)
            if (boundary == "landed") {
                assertTrue("A10: previous landed L must be confirmed by name", previous is ControlStoreResult.Confirmed)
                ...
                assertEquals(saved, disk()); assertEquals(writes, next.storage.writes)
                val fresh = next.control.prepareRetiredNullSettlement(s.targets, s.before, s.executor)
            ...
            successL(executeL(c, ctx), expected)
    ...
    private suspend fun cancelAt(s: RetiredNullSettlement, landed: Boolean) = kotlinx.coroutines.coroutineScope {
        val source = raw(s); seedL(s, source); val c = registerL(s); val pause = ControlStoreTestStorage.Pause()
        if (landed) o.storage.pauseAfterScope = pause else o.storage.pause = pause
        val caller = async { executeL(c, context.copy(ownerUid = s.executor.ownerUid)) }
        try {
            controlTestTimeout("L cancellation gate") { pause.reached.await() }
        ...
        val next = open(file)
        val result = controlTestTimeout("cancelled previous L") { next.control.confirmPrevious(c) }
        if (landed) {
    ...
    private suspend fun beforeRead(s: RetiredNullSettlement, cancel: Boolean) = kotlinx.coroutines.coroutineScope {
        ...
        val c = store.control.prepareRetiredNullSettlement(s.targets, s.before, s.executor)
    ...
    private suspend fun thrownCancellation(s: RetiredNullSettlement) {
        lateinit var faults: BeforeRead
        val store = ControlStoreTestStorage(file) { BeforeRead(it).also { wrapper -> faults = wrapper } }.also { opened += it }
    ...
    @Test fun A10_previousLifetimeCannotRebindApplied() = runReleaseTest {
        val s = both(); seedL(s); val c = registerL(s)
        ...
        controlTestTimeout("L replace tracker linkage") { next.data.edit { it[evidenceKey] = it[evidenceKey]!!.replace(c.ownerTrackingLifetimeId.value, nt.lifetimeId.value) } }
        val saved = disk(); val writes = next.storage.writes
        val result = controlTestTimeout("old L tracker mismatch") { next.control.confirmPrevious(c) }
    ...
    @Test fun A10_previousWitnessWithoutApplied() = runReleaseTest {
        val s = both(); seedL(s); val c = registerL(s)
        successL(executeL(c), ConfirmedEffect.AppliedThisAttempt)
        controlTestTimeout("remove old L evidence") { o.data.edit { it[evidenceKey] = "[]" } }
        o.close(); val next = open(file); val saved = disk(); val writes = next.storage.writes
        val result = controlTestTimeout("old L fixed witness") { next.control.confirmPrevious(c) }
        assertTrue(result is ControlStoreResult.Confirmed)
        assertEquals(ConfirmedEffect.PostconditionConfirmed, (result as ControlStoreResult.Confirmed).effect)
        assertEquals(saved, disk()); assertEquals(writes, next.storage.writes)

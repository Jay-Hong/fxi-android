...
>
> **동결 후 12번 명시적 개정 기록(사용자 GO 2026-09-14, Claude·Codex 설계 합의).** runtime 배선 전 선행조건인 L-4e(topic grant 전달·재개와 live 접근권한 철회 검증)에서 기존 계약이 정하지 않았거나 해석이 갈리는 네 가지를 확정한다.
> (1) **차단의 적용 시점**: 10번(239)의 "현재 유효 접근으로 차단"은 사용 직전 검사 시점에 판정한다. 검사와 사용이 suspension·enqueue·지연 callback 없이 이어지는 한 구간 안에서 무효화 전에 통과한 검사는 그 사용 하나를 허용한다. 구간을 넘는 사용(송신 뒤 답 적용, 지연 continuation, protected read/render)은 각자 다시 검사한다. 무효화 뒤 새로 시작한 검사는 보호된 사용 0이다.
> (2) **인증 실패로 끝난 topic 명령의 재개**: 현재 binding·grant에서 인증 때문에 종료한 topic 명령은, S1 인증 계층이 검증한 credential 회복 전이 1건당 새 명령 예산을 한 번 연다. 같은 회복의 중복 통지는 소비하지 않고, premium 거부·신원 은퇴 잠금과 다른 terminal 거부를 해제하지 않으며, 연결·control lane·in-flight·floor 규칙과 D3 재연결 예산을 따른다. 회복 전이 신호는 9번의 미충족 항목(credential만 회복될 때의 자동 재개)으로 S1이 소유하며, 그 신호가 없으면 이 재개도 일어나지 않는다. iOS v2.0.0은 이 계기로 명령을 재개하지 않으며, 이 차이는 의도적 개선이다.
